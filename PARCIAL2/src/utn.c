 ...
 */
int isValidDocumento(char* string, int len)
{
    int i ;
    int result = ERROR;
 ...
 */
static int getAlfanumerico(char* text, int len)
{
    int result = ERROR;
    char aux[4096];
    ...
    if(text!= NULL && len>0)
    {
        if(getString(aux,sizeof(aux))== FALSE && isValidDocumento(aux,sizeof(aux)) && (strnlen(aux,sizeof(aux))<len))
        {
...
int utn_getIdentityDocument(char *name, int len, char *message, char *errorMessage, int retries)
{
    ...
    if (name != NULL && message != NULL && errorMessage != NULL	&& retries > 0)
    {
        ...
        {
            printf("%s", message);
            if (getAlfanumerico(auxName, sizeof(auxName)) == FALSE && strnlen(auxName, sizeof(auxName)) < len)
            {
                result = FALSE;
