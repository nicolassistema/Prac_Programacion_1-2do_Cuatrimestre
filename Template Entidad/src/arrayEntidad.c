...
int utn_chargeArray (Entidad *list, int len)
{
  ...
  if (list != NULL && len > 0)
      ...
      if (utn_getAceptaRechaza ("\nAcepta el ingreso? s/n: ", "ERROR al ingresar opcion. \n", 's', 'n')==1)
	{
	  ...
	  if(utn_addArray (list, len, auxEmployee.id, auxEmployee.name, auxEmployee.lastName, auxEmployee.salary, auxEmployee.sector)==0)
	  {
	      retorno = 0;
	  }
	  printf ("\n*****************************************************");
 ...
 *
 */
int utn_bufferToArray(Entidad *list, Entidad *listBuffer, int len, int id)
{
  int i;
  int retorno = -1;
...
int utn_updateArray (Entidad *list, int len)
{
  ...
  if (list != NULL && len > 0)
    {
      utn_getNumero ("\nIngrese ID del empleado: ", "\nError al ingresar. ", &bufferID, 1, QTY_ENTIDAD, 5);
      if (utn_findArrayById (list, len, bufferID) != -1)
	{
	  retorno = 0;
	  utn_arrayToBuffer(list, &auxEmployee,  len,  bufferID);
	  do
	     {
	      utn_printForId  (&auxEmployee,len,  bufferID);
	      printf ("\n1-Nombre\n"
		      "2-Apellido\n"
	      	      "3-Salario\n"
	      	      "4-Sector\n"
	 	  ...
	 	  switch (opcion)
	 	   ...
	 	   case 5:
	 		 if(utn_getAceptaRechaza ("\nGrabar cambios? s/n: ", "ERROR al ingresar opcion. \n", 's', 'n')==1)
	 		   {
	 		    utn_bufferToArray (list, &auxEmployee,  len, bufferID);
	 		    printf ("\n*************************************\n");
...
float utn_promedio(Entidad* list, int len)
{
  ...
  if (list != NULL && len > 0)
      ...
      for (i = 0; i < len; i++)
	{
      ...
      retorno = utn_sumaTotal (list, len) / contador;
    }
  return retorno;
}
